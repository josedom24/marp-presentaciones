...
## Reutilización y organización de escenarios

---

## ¿Qué es un módulo?

> Un **módulo** es un conjunto de archivos de configuración que agrupa recursos relacionados, de forma que puedan **reutilizarse, organizarse y parametrizarse** fácilmente.
...
Define la infraestructura una sola vez y úsala en **múltiples entornos**: desarrollo, pruebas, producción
...
Estructura configuraciones grandes en **partes manejables**: un módulo para VMs, otro para redes…
...
Garantiza que recursos similares se creen siempre de forma **coherente y predecible**
...
## Módulos: analogía con funciones
...
```python
def crear_vm(nombre, memoria, imagen):
    # lógica interna
    return ip_asignada
```
...
- **Variables** → valores de entrada
- **Outputs** → valores de retorno
- **Resources internos** → lógica interna
...
## Ejemplo: escenario con módulos (`escenario.tf`)

```hcl
locals {
  networks = {
    red-nat = { mode = "nat", addresses = ["192.168.100.0/24"], dhcp = true }
    red-aislada = { mode = "none", addresses = ["10.0.0.0/24"],  dhcp = false }
  }
  servers = {
    server1 = {
      name = "server1", memory = 1024, vcpu = 1
      base_image    = "debian13-base.qcow2"
      user_data     = "cloud-init/server1/user-data.yaml"
      network_config = "cloud-init/server1/network-config.yaml"
      network = { network_name = "red-nat", wait_for_lease = true }
    }
  }
}
```
...
<span>💡</span><div>Con módulos, añadir una nueva máquina solo requiere <strong>una entrada más</strong> en <code>local.servers</code> — sin crear nuevos ficheros <code>mainX.tf</code>.</div>
</div>

---

## Sin módulos vs. con módulos

<div class="cols-2" style="margin-top:0.8rem">

<div class="card card-red">

### Sin módulos — escenario con 3 VMs

- Crear `main1.tf`, `main2.tf`, `main3.tf`
- Crear `cloud-init/server1/`, `cloud-init/server2/`, `cloud-init/server3/`
- Modificar `output.tf` para las 3 máquinas

**Repetitivo, propenso a errores, difícil de mantener**

</div>

<div class="card card-green">

### Con módulos — escenario con 3 VMs

- Añadir 3 entradas en `local.servers` del fichero `escenario.tf`
- Crear `cloud-init/serverX/` para cada máquina

**El módulo se encarga del resto de forma automática**
